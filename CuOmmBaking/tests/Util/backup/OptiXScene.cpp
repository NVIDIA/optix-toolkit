...
uint32_t getIndexFormatSizeInBytes( ommBaking::IndexFormat format )
{
    switch( format )
    {
...
OptixResult OptixOmmScene::build( OptixDeviceContext context, const char* ptxInput, const size_t ptxInputize, const OptixOmmArray& optixOmm, const ommBaking::BakeInputDesc* ommBuildInput, unsigned int numBuildInputs )
{
    OPTIX_CHECK( destroy() );
...
OptixResult OptixOmmScene::buildGAS( const OptixOmmArray& optixOmm, const ommBaking::BakeInputDesc* ommBuildInput, unsigned int numBuildInputs )
{
    ...
    for( unsigned int i = 0; i < numBuildInputs; ++i )
    {
        ...
        {
            case ommBaking::TexCoordFormat::UV32_FLOAT2:
        ...
        optixBuildInput.triangleArray.indexStrideInBytes = ommBuildInput[i].indexStrideInBytes;
        ...
        switch( ommBuildInput[i].transformFormat )
        {
            break;
            case :
        ...
        optixBuildInput.triangleArray.transformFormat = transformFormat;
        optixBuildInput.triangleArray.preTransform    = ommBuildInput[i].preTransform;
#endif
    ...
    if( numBuildInputs )
    {
...

        {
            ...
            OPTIX_CHECK( optixAccelComputeMemoryUsage( m_context, &accelOptions, buildInputs.data(), (uint32_t)buildInputs.size(), &gasBufferSizes ) );


            CUDA_CHECK( m_gas.alloc( gasBufferSizes.outputSizeInBytes ) );
            CUDA_CHECK( tmp.alloc( gasBufferSizes.tempSizeInBytes ) );

            OPTIX_CHECK( optixAccelBuild( m_context, 0, &accelOptions, buildInputs.data(), (uint32_t)buildInputs.size(), tmp.get(), tmp.byteSize(), m_gas.get(), m_gas.byteSize(), &m_gasHandle, nullptr, 0 ) );
        }
...

        {
            ...
            CUDA_CHECK( tmp.allocIfRequired( iasBufferSizes.tempSizeInBytes ) );

            OPTIX_CHECK( optixAccelBuild( m_context, 0, &accelOptions, &optixBuildInput, 1, tmp.get(), tmp.byteSize(), m_ias.get(), m_ias.byteSize(), &m_iasHandle, nullptr, 0 ) );
        }
...
OptixResult OptixOmmScene::buildSBT( const ommBaking::BakeInputDesc* ommBuildInput, unsigned int numBuildInputs )
{
    // set up SBT
    ...
    for( unsigned int i = 0; i < numBuildInputs; ++i )
    {
        for( unsigned int j = 0; j < ommBuildInput[i].numTextures; ++j )
        {
            ...
            if( ommBuildInput[i].textures )
            {
                ...
                data.texture.alphaCutoff = ommBuildInput[i].textures[j].cuda.alphaCutoff;
                data.texture.opacityCutoff = ommBuildInput[i].textures[j].cuda.opaqueCutoff;
                ...
                data.texture.firstChannelAlpha = ( chanDesc.w == 0 );
