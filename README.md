...
- Create a directory called `build` and `cd` to that directory.
  ...
  cd build
  ```
- Configure the toolkit using CMake, optionally [specifying the location of the OptiX SDK](optix-toolkit#specifying-the-location-of-the-optix-sdk).
...
If you wish to customize the build of the toolkit, see the section on [options](optix-toolkit#optix-toolkit-options).
...
-------- | ---- | ------- | -----------
`OTK_USE_VCPKG` | `BOOL` | `ON` | Use [vcpkg](https://vcpkg.io/) for [dependencies](optix-toolkit#third-party-libraries).
...
`OTK_FETCH_CONTENT` | `BOOL` | `ON` | Use [FetchContent](https://cmake.org/cmake/help/latest/module/FetchContent.html) for [dependencies](optix-toolkit#third-party-libraries) if `OTK_USE_VCPKG` is `OFF`.
...
The toolkit can automatically obtain these third party libraries in one of two ways: via a [vcpkg](optix-toolkit#vcpkg)
manifest or via [FetchContent](optix-toolkit#fetchcontent) as described below.
...
cause the toolkit to use [FetchContent](optix-toolkit#fetchcontent) for third party libraries.
