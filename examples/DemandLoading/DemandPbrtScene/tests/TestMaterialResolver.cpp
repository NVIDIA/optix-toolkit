...
constexpr const char* ROUGHNESS_MAP_PATH{ "roughnessMap.png" };
...
#ifdef OTK_USE_MDL
std::shared_ptr<const otk::pbrt::PbrtMaterial> pbrtMixMaterialWithImagemapAmountTexture( const std::string& valueType )
{
    std::shared_ptr<otk::pbrt::PbrtMaterial> material{ std::make_shared<otk::pbrt::PbrtMaterial>() };
    material->type = "mix";
    addString( material->params, "namedmaterial1", "front" );
    ...
    material->graph.namedMaterials["front"] = pbrtNamedMatteMaterial( "front", 0.2f );
    material->graph.namedMaterials["back"]  = pbrtNamedMatteMaterial( "back", 0.8f );
    addPbrtImagemapTexture( *material, "amount", valueType, "weight", "mix-weight.exr" );
    return material;
}

std::shared_ptr<const otk::pbrt::PbrtMaterial> pbrtMixMaterialWithImagemapAmountTexture()
{
    return pbrtMixMaterialWithImagemapAmountTexture( "float" );
}
...
#ifdef OTK_USE_MDL
void usePbrtGlossyImagemapMaterial( GeometryInstance& geom, const std::string& type )
{
    geom.groups[0].pbrtMaterial = pbrtImagemapMaterialOfType( type, "Ks", "spectrum", SPECULAR_MAP_PATH );
}

void usePbrtAxisRoughnessImagemapMaterial( GeometryInstance& geom, const std::string& type )
{
    std::shared_ptr<otk::pbrt::PbrtMaterial> material{ std::make_shared<otk::pbrt::PbrtMaterial>() };
    material->type = type;
    material->params.AddTexture( "uroughness", "pbrt-texture" );
    material->params.AddTexture( "vroughness", "pbrt-texture" );
    otk::pbrt::PbrtTexture texture{};
    texture.name      = "pbrt-texture";
    texture.valueType = "float";
    texture.type      = "imagemap";
    addString( texture.params, "filename", ROUGHNESS_MAP_PATH );
    material->graph.textures["float:pbrt-texture"] = std::move( texture );
    geom.groups[0].pbrtMaterial                    = std::move( material );
}
...
void usePbrtMixMaterialWithColorImagemapAmountTexture( GeometryInstance& geom )
{
    geom.groups[0].pbrtMaterial = pbrtMixMaterialWithImagemapAmountTexture( "color" );
}
#endif
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedSubsurfaceMaterialFamiliesCompileShaders )
{
    m_options.useMdlMaterials = true;
    GeometryInstance subsurfaceGeom{ m_geom };
    GeometryInstance kdSubsurfaceGeom{ m_geom };
    usePbrtMaterialOfType( subsurfaceGeom, "subsurface" );
    usePbrtMaterialOfType( kdSubsurfaceGeom, "kdsubsurface" );
    const uint_t            proxyGeomId1{ 1111U };
    const uint_t            proxyGeomId2{ 2222U };
    const uint_t            proxyMaterialId1{ 4444U };
    const uint_t            proxyMaterialId2{ 5555U };
    const uint_t            stableSbtOffset1{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 11U };
    const uint_t            stableSbtOffset2{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 12U };
    const MdlMaterialShader expectedShader1{ 8U, 1U };
    const MdlMaterialShader expectedShader2{ 9U, 1U };
    EXPECT_CALL( *m_loader, add() ).WillOnce( Return( proxyMaterialId1 ) ).WillOnce( Return( proxyMaterialId2 ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId1, subsurfaceGeom, m_sync ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId2, kdSubsurfaceGeom, m_sync ) );
    EXPECT_CALL( *m_loader, requestedMaterialIds() ).WillOnce( Return( std::vector<uint_t>{ proxyMaterialId1, proxyMaterialId2 } ) );
    {
        InSequence sequence;
        EXPECT_CALL( *m_programGroups, getMdlMaterialSbtOffset( _ ) ).WillOnce( Return( stableSbtOffset1 ) );
        EXPECT_CALL( *m_programGroups, realizeMdlMaterialShader( _, 1U ) ).WillOnce( Return( expectedShader1 ) );
        EXPECT_CALL( *m_loader, remove( proxyMaterialId1 ) ).Times( 1 );
        EXPECT_CALL( *m_programGroups, getMdlMaterialSbtOffset( _ ) ).WillOnce( Return( stableSbtOffset2 ) );
        EXPECT_CALL( *m_programGroups, realizeMdlMaterialShader( _, 2U ) ).WillOnce( Return( expectedShader2 ) );
        EXPECT_CALL( *m_loader, remove( proxyMaterialId2 ) ).Times( 1 );
    }
    EXPECT_CALL( *m_loader, clearRequestedMaterialIds() ).Times( 1 );
    ...
    EXPECT_EQ( mdlReadyState( proxyMaterialId1, 1U ), m_sync.materialStates[proxyMaterialId1] );
    EXPECT_EQ( mdlReadyState( proxyMaterialId2, 2U ), m_sync.materialStates[proxyMaterialId2] );
    expectMdlMaterialShader( m_sync, proxyMaterialId1, expectedShader1 );
    expectMdlMaterialShader( m_sync, proxyMaterialId2, expectedShader2 );
...
{
    m_options.useMdlMaterials = true;
    const char* const   gapTypes[]  = { "fourier", "hair", "measured" };
    ...
    EXPECT_CALL( *m_programGroups, getRealizedMaterialSbtOffset( _ ) ).Times( 3 ).WillRepeatedly( Return( +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START ) );
    ...
    const MaterialResolverStats stats{ m_resolver->getStatistics() };
    EXPECT_EQ( 3U, stats.numRequestedMaterialPages );
    EXPECT_EQ( 3U, stats.numMdlFallbackShaders );
    EXPECT_EQ( 0U, stats.numGeneratedMdlMaterialCompileRequests );
    EXPECT_EQ( 0U, stats.numFourierBsdfTableResourcesResolved );
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedMixMaterialBindsAmountDemandTexture )
{
    m_options.useMdlMaterials = true;
    ...
    const uint_t      proxyGeomId{ 1111 };
    const uint_t      proxyMaterialId{ 4444U };
    const uint_t      amountTextureId{ 555U };
    const uint_t      stableSbtOffset{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 12U };
    MdlMaterialShader expectedShader{ 8U, 1U };
    ASSERT_TRUE( setMdlMaterialTextureBinding( expectedShader, MDL_MATERIAL_MIX_AMOUNT_TEXTURE_BINDING_INDEX, amountTextureId,
                                               make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ) );
    EXPECT_CALL( *m_loader, add() ).WillOnce( Return( proxyMaterialId ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId, m_geom, m_sync ) );
    EXPECT_CALL( *m_loader, requestedMaterialIds() ).WillOnce( Return( std::vector<uint_t>{ proxyMaterialId } ) );
    EXPECT_CALL( *m_demandTextureCache, createLinearTextureFromFile( EndsWith( "mix-weight.exr" ), false ) )
        .WillOnce( Return( amountTextureId ) );
    EXPECT_CALL( *m_programGroups,
                 getMdlMaterialSbtOffset( hasGeometryInstance(
                     hasAll( hasMaterialFlags( MaterialFlags::NONE ),
                             hasMdlTextureBinding( MDL_MATERIAL_MIX_AMOUNT_TEXTURE_BINDING_INDEX, amountTextureId,
                                                   make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ) ) ) ) )
        .WillOnce( Return( stableSbtOffset ) );
    EXPECT_CALL( *m_programGroups,
                 realizeMdlMaterialShader(
                     hasGeometryInstance( hasAll( hasMaterialFlags( MaterialFlags::NONE ),
                                                  hasMdlTextureBinding( MDL_MATERIAL_MIX_AMOUNT_TEXTURE_BINDING_INDEX,
                                                                        amountTextureId, make_float3( 1.0f, 1.0f, 1.0f ),
                                                                        make_float3( 0.0f, 0.0f, 0.0f ) ) ) ),
                     1U ) )
        .WillOnce( Return( expectedShader ) );
    EXPECT_CALL( *m_loader, remove( proxyMaterialId ) ).Times( 1 );
    ...
    EXPECT_EQ( MaterialResolution::FULL, result );
    EXPECT_EQ( amountTextureId, m_sync.minDiffuseTextureId );
    EXPECT_EQ( amountTextureId, m_sync.maxDiffuseTextureId );
    ASSERT_LT( proxyMaterialId, m_sync.realizedMaterials.size() );
    EXPECT_EQ( fakeUVs, m_sync.realizedUVs.back() );
    EXPECT_EQ( fakeNormals, m_sync.realizedNormals.back() );
    ASSERT_LT( proxyMaterialId, m_sync.materialStates.size() );
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedMixMaterialBindsColorAmountDemandTexture )
{
    m_options.useMdlMaterials = true;
    usePbrtMixMaterialWithColorImagemapAmountTexture( m_geom );
    TriangleUVs*     fakeUVs{ reinterpret_cast<TriangleUVs*>( 0xdeadbeefULL ) };
    TriangleNormals* fakeNormals{ reinterpret_cast<TriangleNormals*>( 0xbaadf00dULL ) };
    m_geom.devUVs     = fakeUVs;
    m_geom.devNormals = fakeNormals;
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedMixMaterialWithNonScalarRgbAmountCompilesShader )
{
    m_options.useMdlMaterials = true;
    ...
    EXPECT_CALL( *m_loader, requestedMaterialIds() ).WillOnce( Return( std::vector<uint_t>{ proxyMaterialId } ) );
    EXPECT_CALL( *m_programGroups, getMdlMaterialSbtOffset( _ ) ).WillOnce( Return( stableSbtOffset ) );
    EXPECT_CALL( *m_programGroups, realizeMdlMaterialShader( _, 1U ) ).WillOnce( Return( expectedShader ) );
    EXPECT_CALL( *m_loader, remove( proxyMaterialId ) ).Times( 1 );
    EXPECT_CALL( *m_loader, clearRequestedMaterialIds() ).Times( 1 );
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedPlasticMaterialBindsGlossyDemandTexture )
{
    m_options.useMdlMaterials = true;
    usePbrtGlossyImagemapMaterial( m_geom, "plastic" );
    TriangleUVs*     fakeUVs{ reinterpret_cast<TriangleUVs*>( 0xdeadbeefULL ) };
    TriangleNormals* fakeNormals{ reinterpret_cast<TriangleNormals*>( 0xbaadf00dULL ) };
    m_geom.devUVs     = fakeUVs;
    m_geom.devNormals = fakeNormals;
    const uint_t                   proxyGeomId{ 1111U };
    const uint_t                   proxyMaterialId{ 4444U };
    const uint_t                   specularTextureId{ 333U };
    const uint_t                   stableSbtOffset{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 12U };
    const MdlMaterialShader        expectedShader{ 8U, 1U };
    const PbrtDemandTextureBinding specularBinding{ pbrtColorTextureBinding( *m_geom.groups[0].pbrtMaterial, "Ks" ) };
    EXPECT_CALL( *m_loader, add() ).WillOnce( Return( proxyMaterialId ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId, m_geom, m_sync ) );
    EXPECT_CALL( *m_loader, requestedMaterialIds() ).WillOnce( Return( std::vector<uint_t>{ proxyMaterialId } ) );
    EXPECT_CALL( *m_demandTextureCache, createLinearTextureFromFile( StrEq( specularBinding.fileName ), true ) )
        .WillOnce( Return( specularTextureId ) );
    EXPECT_CALL( *m_programGroups,
                 getMdlMaterialSbtOffset( hasGeometryInstance(
                     hasAll( hasMaterialFlags( MaterialFlags::NONE ),
                             hasMdlTextureBinding( MDL_MATERIAL_KS_TEXTURE_BINDING_INDEX, specularTextureId,
                                                   make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ) ) ) ) )
        .WillOnce( Return( stableSbtOffset ) );
    EXPECT_CALL( *m_programGroups,
                 realizeMdlMaterialShader(
                     hasGeometryInstance( hasAll( hasMaterialFlags( MaterialFlags::NONE ),
                                                  hasMdlTextureBinding( MDL_MATERIAL_KS_TEXTURE_BINDING_INDEX,
                                                                        specularTextureId, make_float3( 1.0f, 1.0f, 1.0f ),
                                                                        make_float3( 0.0f, 0.0f, 0.0f ) ) ) ),
                     1U ) )
        .WillOnce( Return( expectedShader ) );
    EXPECT_CALL( *m_loader, remove( proxyMaterialId ) ).Times( 1 );
    ...
    EXPECT_EQ( MaterialResolution::FULL, result );
    EXPECT_EQ( specularTextureId, m_sync.minDiffuseTextureId );
    EXPECT_EQ( specularTextureId, m_sync.maxDiffuseTextureId );
    ASSERT_LT( proxyMaterialId, m_sync.realizedMaterials.size() );
    EXPECT_EQ( fakeUVs, m_sync.realizedUVs.back() );
    EXPECT_EQ( fakeNormals, m_sync.realizedNormals.back() );
    ASSERT_LT( proxyMaterialId, m_sync.materialStates.size() );
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedSubstrateMaterialBindsRoughnessDemandTexture )
{
    m_options.useMdlMaterials = true;
    usePbrtAxisRoughnessImagemapMaterial( m_geom, "substrate" );
    TriangleUVs*     fakeUVs{ reinterpret_cast<TriangleUVs*>( 0xdeadbeefULL ) };
    TriangleNormals* fakeNormals{ reinterpret_cast<TriangleNormals*>( 0xbaadf00dULL ) };
    m_geom.devUVs     = fakeUVs;
    m_geom.devNormals = fakeNormals;
    const uint_t                   proxyGeomId{ 1111U };
    const uint_t                   proxyMaterialId{ 4444U };
    const uint_t                   roughnessTextureId{ 333U };
    const uint_t                   stableSbtOffset{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 12U };
    const MdlMaterialShader        expectedShader{ 8U, 1U };
    const PbrtDemandTextureBinding uRoughnessBinding{
        pbrtFloatTextureBinding( *m_geom.groups[0].pbrtMaterial, "uroughness" ) };
    const PbrtDemandTextureBinding vRoughnessBinding{
        pbrtFloatTextureBinding( *m_geom.groups[0].pbrtMaterial, "vroughness" ) };
    EXPECT_EQ( uRoughnessBinding.fileName, vRoughnessBinding.fileName );
    EXPECT_CALL( *m_loader, add() ).WillOnce( Return( proxyMaterialId ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId, m_geom, m_sync ) );
    EXPECT_CALL( *m_loader, requestedMaterialIds() ).WillOnce( Return( std::vector<uint_t>{ proxyMaterialId } ) );
    EXPECT_CALL( *m_demandTextureCache, createLinearTextureFromFile( StrEq( uRoughnessBinding.fileName ), true ) )
        .Times( 2 )
        .WillRepeatedly( Return( roughnessTextureId ) );
    EXPECT_CALL( *m_programGroups,
                 getMdlMaterialSbtOffset( hasGeometryInstance(
                     hasAll( hasMaterialFlags( MaterialFlags::NONE ),
                             hasMdlTextureBinding( MDL_MATERIAL_UROUGHNESS_TEXTURE_BINDING_INDEX, roughnessTextureId,
                                                   make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ),
                             hasMdlTextureBinding( MDL_MATERIAL_VROUGHNESS_TEXTURE_BINDING_INDEX, roughnessTextureId,
                                                   make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ) ) ) ) )
        .WillOnce( Return( stableSbtOffset ) );
    EXPECT_CALL( *m_programGroups,
                 realizeMdlMaterialShader(
    ...
    EXPECT_EQ( MaterialResolution::FULL, result );
    EXPECT_EQ( roughnessTextureId, m_sync.minDiffuseTextureId );
    EXPECT_EQ( roughnessTextureId, m_sync.maxDiffuseTextureId );
    ASSERT_LT( proxyMaterialId, m_sync.realizedMaterials.size() );
    EXPECT_EQ( fakeUVs, m_sync.realizedUVs.back() );
    EXPECT_EQ( fakeNormals, m_sync.realizedNormals.back() );
    ASSERT_LT( proxyMaterialId, m_sync.materialStates.size() );
...
TEST_F( TestMaterialResolverRequestedProxyIds, requestedGeneratedKdSubsurfaceMaterialBindsDiffuseDemandTexture )
{
    m_options.useMdlMaterials = true;
    usePbrtDiffuseImagemapMaterial( m_geom, "kdsubsurface" );
    TriangleUVs*     fakeUVs{ reinterpret_cast<TriangleUVs*>( 0xdeadbeefULL ) };
    TriangleNormals* fakeNormals{ reinterpret_cast<TriangleNormals*>( 0xbaadf00dULL ) };
    m_geom.devUVs     = fakeUVs;
    m_geom.devNormals = fakeNormals;
    ...
    const uint_t                   stableSbtOffset{ +ProgramGroupIndex::HITGROUP_REALIZED_MATERIAL_START + 12U };
    const PbrtDemandTextureBinding diffuseBinding{ pbrtColorTextureBinding( *m_geom.groups[0].pbrtMaterial, "Kd" ) };
    const MdlMaterialShader        expectedShader{ 8U, 1U, linearTextureId, make_float3( 1.0f, 1.0f, 1.0f ),
                                                   make_float3( 0.0f, 0.0f, 0.0f ) };
    EXPECT_CALL( *m_demandTextureCache, hasDiffuseTextureForFile( StrEq( diffuseBinding.fileName ) ) ).WillOnce( Return( false ) );
    EXPECT_CALL( *m_loader, add() ).WillOnce( Return( proxyMaterialId ) );
    ASSERT_FALSE( m_resolver->resolveMaterialForGeometry( proxyGeomId, m_geom, m_sync ) );
    ...
    EXPECT_CALL( *m_programGroups,
                 getMdlMaterialSbtOffset( hasGeometryInstance(
                     hasAll( hasMaterialFlags( MaterialFlags::DIFFUSE_MAP | MaterialFlags::DIFFUSE_MAP_ALLOCATED ),
                             hasDiffuseTextureId( diffuseTextureId ),
                             hasMdlTextureBinding( MDL_MATERIAL_KD_TEXTURE_BINDING_INDEX, linearTextureId,
                                                   make_float3( 1.0f, 1.0f, 1.0f ), make_float3( 0.0f, 0.0f, 0.0f ) ) ) ) ) )
        .WillOnce( Return( stableSbtOffset ) );
    EXPECT_CALL( *m_programGroups,
                 realizeMdlMaterialShader(
