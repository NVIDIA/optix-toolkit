...
#include <OptiXToolkit/NeuralTextures/NtcTextureSet.h>
...
ntcTex2DGrad( T_VEC_OUT& out, NtcTextureSet* &nts, const DeviceContext& context, TextureSampler* sampler, float u, float v, float2 ddx, float2 ddy, float2 xi )
{
    // Get the NtcTextureSet metadata
    if( !sampler || !sampler->extraData )
        return false;

    nts = reinterpret_cast<NtcTextureSet*>( sampler->extraData );
    ...
    if( sampler->desc.isSparseTexture )
    {
        ...
        const NtcColorMipConstants& colorMip = nts->constants.colorMips[mip];
    ...
    const int mipWidth = nts->constants.imageWidth >> mip;
    const int mipHeight = nts->constants.imageHeight >> mip;
    const int x = clamp( (int)(uv.x * mipWidth + jitter.x), 0, mipWidth - 1 );
    const int y = clamp( (int)(uv.y * mipHeight + jitter.y), 0, mipHeight - 1 );

    return SampleTextureSet<T_VEC_OUT>( out, nts->constants, sampler->texture, nts->latentFeatures, 
        nts->latentWidth, nts->latentHeight, nts->d_mlpWeights, x, y, mip );
}


template <class T_VEC_OUT> D_INLINE bool
ntcTex2DGrad( T_VEC_OUT& out, NtcTextureSet* &nts, const DeviceContext& context, unsigned int textureId, float u, float v, float2 ddx, float2 ddy, float2 xi )
{
    bool resident;
    TextureSampler* sampler = reinterpret_cast<TextureSampler*>( pagingMapOrRequest( context, textureId, &resident ) );
    return ntcTex2DGrad<T_VEC_OUT>( out, nts, context, sampler, u, v, ddx, ddy, xi );
}
...
{
    NtcTextureSet* nts;
    return ntcTex2DGrad<T_VEC_OUT>( out, nts, context, textureId, u, v, ddx, ddy, xi );
}


template <class T_VEC_OUT> D_INLINE bool
ntcTex2DGradUdim( T_VEC_OUT& out, NtcTextureSet* &nts, const DeviceContext& context, unsigned int textureId, float x, float y, float2 ddx, float2 ddy, float2 xi )
{
    ...
    if( !useBaseTexture )
    {
        ...
        isResident = ntcTex2DGrad<T_VEC_OUT>( out, nts, context, subTexId, sx, sy, ddx, ddy, xi );
    ...
    return ntcTex2DGrad<T_VEC_OUT>( out, nts, context, bsmp, x, y, ddx, ddy, xi );
}
...
{
    NtcTextureSet* nts;
    return ntcTex2DGradUdim<T_VEC_OUT>( out, nts, context, textureId, x, y, ddx, ddy, xi );
}
